...
namespace SDE.Editor.Engines.Parsers.Yaml
{
    public class YamlParser
    {
        ...
        private class YamlFileData
        {
            ...
            {
                Data = file.IsExtension(".dat") ? File.ReadAllBytes(file) : IOHelper.ReadAllBytes(file);
                FileName = file;
                LineNumber = 1;
            }
            ...
            public void Trim()
            {
                while (CanRead)
                {
                    ...
                    if (c == '#')
                    {   // Skip comment line when trimming end of key or word
                        while (CanRead)
                        {
                            if (Data[Position++] == '\n')
                            {
            ...
            public string ReadValue()
            {
                ...
                else if (Data[Position] == '|' || Data[Position] == '>')
                {
                    ...
                    while (CanRead)
                    {
                        ...
                        { // Needs to be checked after to make an exception for empty lines
                            break;
                        }
            ...
            public Exception GetException(string reason)
            {
                ...
                try
                {
                    ...
                    if (length > 0)
                    {
                        lastLine = (SdeAppConfiguration.EncodingServer ?? Encoding.Default).GetString(Data, oldPosition, length);
                    }
                }
                finally
                {
            ...
            while (file.CanRead)
            {
                ...
                switch (c)
                {
                    ...
                    case ':':
                        ...
                        else if (file.Data[file.Position] == '[')
                        { // Aggregate parsing, does not support multi-line
                            file.Position++;
                        ...
                        {   // KeyValue, get the line number first!
        ...
        public void Remove<TKey>(AbstractDb<TKey> db, Func<TKey, string> key2string = null)
        {
            ...
            foreach (GroupCommand<TKey, ReadableTuple<TKey>> command in db.Table.Commands.GetUndoCommands().OfType<GroupCommand<TKey, ReadableTuple<TKey>>>())
            {
                foreach (DeleteTuple<TKey, ReadableTuple<TKey>> deleteCommand in command.Commands.OfType<DeleteTuple<TKey, ReadableTuple<TKey>>>())
                {
                    Delete(key2string(deleteCommand.Key));
                }

                foreach (ChangeTupleKey<TKey, ReadableTuple<TKey>> changeTupleKeyCommand in command.Commands.OfType<ChangeTupleKey<TKey, ReadableTuple<TKey>>>())
                {
            ...
            foreach (ChangeTupleKey<TKey, ReadableTuple<TKey>> command in db.Table.Commands.GetUndoCommands().OfType<ChangeTupleKey<TKey, ReadableTuple<TKey>>>())
            {
                // If the key was changed, the old key must be removed
