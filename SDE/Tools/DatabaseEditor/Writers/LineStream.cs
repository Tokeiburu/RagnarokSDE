...
namespace SDE.Tools.DatabaseEditor.Writers {
	/// <summary>
	...
	public abstract class LineStream<T> {
		...
		public bool? HasDuplicateIds() {
			if (_ids.All(p => p.ToString() == Default.ToString()))
				return null;

			HashSet<T> ids = new HashSet<T>();

			foreach (T id in _ids.Where(p => p.ToString() != Default.ToString())) {
				if (!ids.Add(id))
					return true;
			}
		...
		public void WriteFile(string path) {
			...
			File.WriteAllText(path, builder.ToString(), EncodingService.DisplayEncoding);
		}
