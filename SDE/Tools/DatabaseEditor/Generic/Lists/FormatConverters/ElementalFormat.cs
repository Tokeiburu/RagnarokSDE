		...
		private void _comboBox_SelectionChanged(object sender, SelectionChangedEventArgs e) {
			...
			try {
				if (_tab.List.SelectedItem != null) {
					...
					_tab.Table.Commands.StoreAndExecute(new ChangeTupleProperty<int, ReadableTuple<int>>((ReadableTuple<int>)_tab.List.SelectedItem, _attribute, value.ToString(CultureInfo.InvariantCulture)));
				}
			}
			catch (Exception err) {
