...
namespace SDE.Tools.DatabaseEditor.Generic.CustomControls {
	public partial class CustomItemGroupDisplay<TKey> : FormatConverter<TKey, ReadableTuple<TKey>> {
		...
		private void _miAddDrop_Click(object sender, RoutedEventArgs e) {
			...
			try {
				DropEdit dialog = new DropEdit("", "1", _tab.GetDb<int>(ServerDBs.Items));
		...
		private void _addedItem(ReadableTuple<int> tuple, DbAttribute attribute, int dkey, ReadableTuple<int> dvalue, int newdkey, bool executed) {
			...
			if (executed) {
				...
				var itemView = new ItemView(((GenericDatabase) _tab.Database).GetTable<int>(ServerDBs.ItemGroups), dico, dkey);
				result.Insert(0, itemView);
		...
		private void _miSelect_Click(object sender, RoutedEventArgs e) {
			if (_lv.SelectedItems.Count > 0) {
				TabNavigationEngine.SelectList(ServerDBs.Items, _lv.SelectedItems.Cast<ItemView>().ToList().Select(p => p.ID).ToList());
			}
		}
		private void _miEditDrop_Click(object sender, RoutedEventArgs e) {
			...
			Table<int, ReadableTuple<int>> btable = ((GenericDatabase)_tab.Database).GetTable<int>(ServerDBs.ItemGroups);

			try {
				var selectedItem = (ItemView)_lv.SelectedItem;
				DropEdit dialog = new DropEdit(selectedItem.ID.ToString(CultureInfo.InvariantCulture), selectedItem.Rate.ToString(CultureInfo.InvariantCulture), _tab.GetDb<int>(ServerDBs.Items)) { Element2 = "Frequency" };
				dialog.Owner = WpfUtilities.TopWindow;
		...
		public void Select(List<ItemView> items) {
			TabControl obj = (TabControl)_tab.Parent;
			var tab = WpfUtilities.GetTab(obj, ServerDBs.Items.Filename);

			if (tab != null) {
				Table<int, ReadableTuple<int>> btable = ((GenericDatabase)_tab.Database).GetTable<int>(ServerDBs.Items);
				obj.SelectedItem = tab;
				List<Tuple> tuples = items.Select(p => (Tuple)btable.TryGetTuple(p.ID)).Where(p => p != null).ToList();

				GrfThread.Start(delegate {
					GDbTabWrapper<int, ReadableTuple<int>> rTab = (GDbTabWrapper<int, ReadableTuple<int>>)tab;
					int attemps = 10;

					while (rTab.SearchEngine.IsFiltering && attemps > 0) {
						Thread.Sleep(200);
						attemps--;
					}

					rTab.Dispatch(p => p.SelectItems(tuples));
				});
			}
		}
