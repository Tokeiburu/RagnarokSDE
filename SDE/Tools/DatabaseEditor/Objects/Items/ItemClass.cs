...
namespace SDE.Tools.DatabaseEditor.Objects.Items {
	public class ItemClass {
		...
		public string ResourceName {
			get {
				return EncodingService.GetCurrentString(_resource);
			}
		}
	}
	...
	public sealed class ItemClasses {
		...
		public static ItemClass Footgear = new SubItemClass(ArmorPrimary, "½´Áî", new string[] { "Footgear", "Foot gear" });
		...
		public static SubItemClass GetSubClass(string item) {
			foreach (SubItemClass subClass in Classes.Where(p => p is SubItemClass)) {
				if (ItemHelper.ComparesTo(item, subClass.DisplayDefinitions))
					return subClass;
			}
			return null;
		}
		...
		public static ItemClass GetClass(string item) {
			...
			foreach (PrimaryItemClass primaryClass in Classes.Where(p => p is PrimaryItemClass)) {
				if (primaryClass.DisplayDefinitions == null)
					continue;

				if (ItemHelper.ComparesTo(item, primaryClass.DisplayDefinitions))
					return primaryClass;
			}
			return null;
		}
